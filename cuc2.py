...
class PacketSnifferApp:
    ...
    def setup_ui(self):
        ...
        self.tree = ttk.Treeview(self.table_frame, columns=("No","Source", "Destination", "Protocol", "Length", "Info"), show="headings")
    ...
    def update_display(self, packets=None):
        ...
        for index, packet in enumerate(packets, start=1):  # Đánh số từ 1
            ...
            # Thêm vào bảng với số thứ tự
            self.tree.insert("", "end", values=(index, src_ip, dst_ip, protocol_name, length, info))
    ...
    def view_packet_details(self):
        ...
        if selected_item:
            ...
            text = tk.Text(detail_window, wrap=tk.NONE, font=("Courier", 12))  # Font fixed-width
            ...
            if packet.haslayer(IP):
                packet_details += (
                    "ETH HEADER\n"
                    f"Destination MAC:  {packet.src: <20}\n"
                    f"Source MAC:       {packet.dst: <20}\n"
                    f"Proto:            {packet.type}\n\n"
                    
                    "IP HEADER\n"
                    f"Version:          {packet[IP].version}\n"
                    f"IHL:              {packet[IP].ihl}\n"
                    f"TOS:              {packet[IP].tos}\n"
                    f"Length:           {packet[IP].len}\n"
                    f"ID:               {packet[IP].id}\n"
                    f"Flags:            {packet[IP].flags}\n"
                    f"Offset:           {packet[IP].frag}\n"
                    f"TTL:              {packet[IP].ttl}\n"
                    f"Next Proto:       {packet[IP].proto}\n"
                    f"Checksum:         {packet[IP].chksum}\n"
                    f"Source IP:        {packet[IP].src}\n"
                    f"Dest IP:          {packet[IP].dst}\n\n"
                )

            if packet.haslayer(UDP):
                packet_details += (
                    "UDP HEADER\n"
                    f"Source:           {packet[UDP].sport}\n"
                    f"Dest:             {packet[UDP].dport}\n"
                    f"Length:           {packet[UDP].len}\n"
                    f"Checksum:         {packet[UDP].chksum}\n"
                )

            text.insert(tk.END, packet_details)
        else:
            messagebox.showwarning("View Packet Details", "No packet selected")


    def view_statistics(self):
        # Đếm số lượng từng loại giao thức trong các gói đã bắt
        protocol_counts = Counter(self.get_protocol_name(pkt) for pkt in self.filtered_packets)
        ...
        plt.bar(protocols, counts, color="tomato")
        plt.xlabel("Protocol")
        plt.ylabel("Count")
